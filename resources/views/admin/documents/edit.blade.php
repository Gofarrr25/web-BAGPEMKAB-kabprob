...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 max-w-3xl">
    <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
        <h3 class="font-bold text-gray-800">Form Edit Dokumen (.PDF / .ZIP)</h3>
        ...
        <form action="{{ route('admin.documents.update', $document->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-5">
                ...
                <select name="category" required class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 outline-none bg-white font-semibold">
                    ...
                    @foreach($categories as $cat)
                        <option value="{{ $cat->title }}" {{ old('category', $document->category) == $cat->title ? 'selected' : '' }}>{{ $cat->title }}</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div class="p-4 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50 text-center">
                    ...
                    @if($document->file_path)
                        <div class="mb-2">
                            <a href="{{ asset('storage/'.$document->file_path) }}" target="_blank" class="text-xs text-blue-600 font-semibold hover:underline">Lihat PDF Saat Ini</a>
                        </div>
                    @endif
                    ...
                    <p class="text-xs text-gray-500 mt-2 font-medium">Opsional. Kosongkan jika tidak ingin mengubah.</p>
                ...
                <div class="p-4 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50 text-center">
                    ...
                    @if($document->zip_path)
                        <div class="mb-2">
                            <a href="{{ asset('storage/'.$document->zip_path) }}" class="text-xs text-blue-600 font-semibold hover:underline">Download ZIP Saat Ini</a>
                        </div>
                    @endif
            ...
            <div class="border-t border-gray-100 pt-6 flex gap-4">
                ...
                <button type="submit" class="px-6 py-2 bg-blue-600 text-white font-bold rounded hover:bg-blue-700 transition shadow-md">
                    <i class="fas fa-save mr-1"></i> Update Dokumen
                </button>
            </div>
        </form>
    </div>
