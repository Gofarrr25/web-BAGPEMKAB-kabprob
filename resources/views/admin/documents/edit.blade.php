            ...
            <div class="mb-5">
                ...
                <select name="category" required class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-brand-blue outline-none bg-white font-semibold">
                    ...
                    @php
                        ...
                        $categoryExists = $categories->contains('title', $selectedCategory);
                    @endphp
                    @if($selectedCategory && !$categoryExists)
                        <option value="{{ $selectedCategory }}" selected>{{ $selectedCategory }} (Tidak Aktif)</option>
                    @endif
                    @foreach($categories as $cat)
                        <option value="{{ $cat->title }}" {{ $selectedCategory == $cat->title ? 'selected' : '' }}>
                            {{ $cat->title }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div class="p-4 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50 text-center">
                    ...
                    @if($document->file_path)
                        <div class="mb-3">
                            <a href="{{ asset('storage/' . $document->file_path) }}" target="_blank" class="text-brand-blue hover:underline text-xs font-semibold"><i class="fas fa-external-link-alt"></i> Lihat File Saat Ini</a>
                        </div>
                    @endif
                ...
                <div class="p-4 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50 text-center">
                    ...
                    @if($document->zip_path)
                        <div class="mb-3">
                            <a href="{{ asset('storage/' . $document->zip_path) }}" target="_blank" class="text-brand-blue hover:underline text-xs font-semibold"><i class="fas fa-external-link-alt"></i> Lihat File Saat Ini</a>
                        </div>
                    @endif
