        ...
        <table class="w-full text-left border-collapse min-w-[800px]">
            ...
            <tbody class="text-sm">
                ...
                <tr class="border-b border-gray-50 hover:bg-gray-50/50 transition">
                        ...
                        <div class="flex items-center gap-3">
                            <div class="text-2xl">
                                ...
                                @elseif(in_array($ext, ['doc', 'docx']))
                                    <i class="fas fa-file-word text-brand-blue"></i>
                    ...
                    <td class="px-6 py-4">
                        <span class="bg-brand-blue-light text-brand-blue px-3 py-1 rounded-full text-xs font-bold border border-brand-blue-light">
                            {{ $doc->category }}
                        </span>
                    </td>
                    ...
                    <td class="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                        @php
                            $canEditDelete = !auth()->user()->hasRole('Staf') || $doc->user_id === auth()->id();
                        @endphp
                        @if($canEditDelete)
                        ...
                        @endif
                        <a href="{{ asset('storage/' . $doc->file_path) }}" target="_blank" download class="text-brand-blue hover:text-brand-blue-hover font-bold px-3 py-1.5 rounded-lg border border-brand-blue-light hover:bg-brand-blue-light transition text-xs inline-flex items-center gap-1" title="Unduh File">
                            <i class="fas fa-download"></i> Download {{ strtoupper($ext) }}
                        </a>
                        @if($canEditDelete)
                        <form action="{{ route('admin.documents.destroy', $doc->id) }}" method="POST" class="inline-block" onsubmit="event.preventDefault(); confirmDelete(this, 'Dokumen PPID', 'Data Terpilih', true);">
                            @csrf
                            @method('DELETE')
                        ...
                        @endif
                        @if(!$canEditDelete)
                        <span class="text-gray-400 text-xs italic ml-2"><i class="fas fa-lock mr-1"></i>Akses Terbatas</span>
                        @endif
                    </td>
                </tr>
                @endforeach
