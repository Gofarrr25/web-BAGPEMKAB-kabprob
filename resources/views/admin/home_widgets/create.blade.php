...
@section('page_title', 'Tambah Widget Home')
...
<div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 max-w-4xl mx-auto">
    ...
    <form action="{{ route('admin.home-widgets.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        
        <div>
            <label class="block text-sm font-bold text-gray-700 mb-2">Judul Widget <span class="text-red-500">*</span></label>
            <input type="text" name="title" value="{{ old('title') }}" required class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-brand-blue outline-none transition" placeholder="Contoh: Maklumat Pelayanan">
        </div>
        ...
        <div>
            ...
            <div id="imagePreviewContainer" class="mt-3 hidden relative w-64 aspect-[3/2] border border-gray-200 rounded overflow-hidden">
        ...
        <div>
            ...
            <input type="text" name="link_url" value="{{ old('link_url') }}" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-brand-blue outline-none transition" placeholder="Contoh: https://lapor.go.id atau /halaman-internal">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="number" name="order_index" value="{{ old('order_index', 0) }}" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-brand-blue outline-none transition">
        ...
        <div class="pt-4 border-t border-gray-100 flex justify-end">
            <button type="submit" class="px-6 py-2.5 bg-brand-blue text-white font-bold rounded-lg shadow-md hover:bg-brand-dark transition flex items-center gap-2">
                <i class="fas fa-save"></i> Simpan Widget
            </button>
        </div>
</form>
</div>
