...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 max-w-5xl mx-auto">
    <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
        <h3 class="font-bold text-gray-800"><i class="fas fa-sliders-h text-brand-blue mr-2"></i> Identitas Website & Kelola Konten Footer</h3>
        <span class="bg-brand-blue-light text-brand-blue px-3 py-1 rounded-full text-xs font-bold">Khusus Superadmin</span>
    </div>
    ...
    <div class="p-4 md:p-6">
        <form action="{{ route('admin.settings.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-8 p-5 bg-brand-blue-light/60 border border-brand-blue-light rounded-xl">
                <h4 class="font-bold text-brand-blue mb-4 text-sm flex items-center gap-2">
                    <i class="fas fa-landmark text-brand-blue"></i> Identitas & Logo Utama (Header & Navbar)
                </h4>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div>
                        ...
                        <input type="file" name="site_logo" accept="image/jpeg,image/png,image/webp,image/gif" class="w-full text-xs text-gray-500 file:mr-2 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-brand-blue file:text-white hover:file:bg-brand-blue cursor-pointer">
                    ...
                    <div class="md:col-span-2 space-y-4">
                        <div>
                            ...
                            <input type="text" name="site_name" value="{{ $settings['site_name'] ?? 'Bagian Pemerintahan Kabupaten Probolinggo' }}" required
                                   class="w-full px-3.5 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue outline-none font-bold text-gray-800 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">Deskripsi Singkat SEO</label>
                            <textarea name="site_description" rows="2" class="w-full px-3.5 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue outline-none text-xs">{{ $settings['site_description'] ?? 'Website Resmi Bagian Pemerintahan Sekretariat Daerah Kabupaten Probolinggo.' }}</textarea>
                        </div>
                    </div>
                </div>
            </div>

            <!-- SECTION PENGATURAN KONTEN FOOTER WEBSITE -->
            <div class="mb-8 p-5 bg-amber-50/50 border border-amber-200 rounded-xl">
                <h4 class="font-bold text-amber-900 mb-4 text-sm flex items-center gap-2 border-b border-amber-200 pb-3">
                    <i class="fas fa-shoe-prints text-amber-600"></i> Pengaturan Konten Footer Website
                </h4>

                <div class="space-y-6">
                    
                    <!-- Bagian Footer: Logo & Deskripsi -->
                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-2">Logo Khusus Footer (Opsional)</label>
                        <div class="flex items-center gap-6 mb-2 bg-white p-4 border border-gray-200 rounded-xl shadow-sm">
                            <div class="w-40 h-24 bg-gray-900 rounded-lg p-3 flex items-center justify-center shadow-inner relative overflow-hidden">
                                ...
                                @else
                                    <img src="https://diskominfo.probolinggokab.go.id/backend/gambar/logo_backend.png" alt="Logo Default" class="max-h-full max-w-full object-contain filter invert opacity-50">
                                @endif
                            </div>
                            <div class="flex-1">
                                <input type="file" name="footer_logo" accept="image/jpeg,image/png,image/webp,image/gif" class="w-full text-sm text-gray-500 file:mr-4 file:py-2.5 file:px-5 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-amber-600 file:text-white hover:file:bg-amber-700 cursor-pointer transition">
                                <p class="text-xs text-gray-500 mt-3 leading-relaxed"><i class="fas fa-info-circle text-amber-500 mr-1"></i> Rekomendasi 400 &times; 140 px, rasio 20:7, format PNG transparan. Karena footer menggunakan background gelap, disarankan menggunakan teks terang.</p>
                            </div>
                        </div>
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-2">Teks Deskripsi Profil di Footer</label>
                        <textarea name="footer_description" id="ck-footer-editor" rows="6" class="w-full px-3.5 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 outline-none text-xs leading-relaxed" placeholder="Tuliskan deskripsi profil singkat yang tampil pada footer kiri website...">{{ $settings['footer_description'] ?? 'Website Resmi Bagian Pemerintahan Sekretariat Daerah Kabupaten Probolinggo. Merupakan media informasi elektronik satu pintu meliputi pelayanan publik, dokumen transparansi kinerja, dan informasi umum bagi masyarakat.' }}</textarea>
                    </div>
            ...
            <div class="border-t border-gray-100 mt-6 pt-5 mb-6">
                <button type="submit" class="w-full py-3 bg-green-600 text-white font-bold rounded-lg hover:bg-brand-blue-hover transition shadow text-sm flex items-center justify-center gap-2">
