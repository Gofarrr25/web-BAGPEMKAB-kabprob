...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 max-w-3xl">
    <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex flex-wrap justify-between items-center gap-3">
        <h3 class="font-bold text-gray-800 w-full sm:w-auto">Form Edit Admin & Ubah Password</h3>
        <div class="flex flex-wrap items-center gap-2">
            <span class="bg-yellow-100 text-yellow-800 text-xs px-3 py-1.5 rounded-full font-bold">Edit Admin</span>
    ...
    <div class="p-4 md:p-6">
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="flex-1 space-y-4">
                    ...
                    <div>
                        ...
                        <select name="role" required class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-brand-blue focus:border-brand-blue outline-none transition">
                            ...
                            @endphp
                            @foreach($roles as $r)
                                <option value="{{ $r->name }}" {{ (old('role') ?? $userRole) == $r->name ? 'selected' : '' }}>{{ $r->name }}</option>
                            @endforeach
                        </select>
            ...
            <div class="flex gap-4">
                ...
                <button type="submit" class="px-6 py-2.5 bg-brand-blue text-white font-bold rounded-lg hover:bg-brand-blue-hover transition shadow-md text-sm">Perbarui Data Admin</button>
            </div>
        </form>
    </div>
</div>
